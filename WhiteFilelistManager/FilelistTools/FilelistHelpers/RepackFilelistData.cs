...
namespace WhiteFilelistManager.FilelistTools.FilelistHelpers
{
    ...
    {
        public static void BuildFilelist(FilelistVariables filelistVariables, Dictionary<int, List<byte>> newChunksDict, GameCode gameCode)
        {
                ...
                using (var chunkInfoWriter = new BinaryWriter(chunkInfoStream))
                {
                    using (var chunkDataStream = new MemoryStream())
                    {
                        ...
                        for (int c = 0; c < filelistVariables.TotalChunks; c++)
                        {
                            ...
                            var currentChunkCmp = ZlibFunctions.ZlibCompressBuffer(currentChunk);
                        ...
                        using (var newFilelistChunks = new FileStream(filelistVariables.MainFilelistFile, FileMode.Append, FileAccess.Write))
                        {
            ...
            using (var tmpEntriesStream = new MemoryStream())
            {
                ...
                {
                    using (var newEntryWriter = new BinaryWriter(File.Open(filelistVariables.MainFilelistFile, FileMode.Open, FileAccess.Write)))
                    {
                        ...
                        for (int c = 0; c < filelistVariables.TotalChunks; c++)
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (gameCode.Equals(GameCode.ff132))
                                {
